...
window.MS_UI = (function () {
  ...
  const NAV_CONFIG = {
    administrator: [
      ...
      { id: "geofences", label: "Geofence Manager", icon: "🛡️" },
    ...
    command: [
      ...
      { id: "history", label: "Incident History", icon: "📜" },
  ...
  function getNotificationsForRole(s, role) {
    ...
    return allNotifs.filter((n) => {
      ...
      if (Array.isArray(n.targetRoles) && n.targetRoles.length > 0) {
        return n.targetRoles.includes(r) || n.targetRoles.includes("all");
      }
      if (n.targetRole) {
        return n.targetRole === r || n.targetRole === "all";
      }

      // 2. Strict Role-specific fallback routing by notification type
      if (r === "administrator") {
        return n.type === "system" || n.type === "audit" || n.type === "operational";
      }
      if (r === "command") {
        return n.type === "threat" || n.type === "alert" || (n.type === "mission" && (n.title.includes("Accepted") || n.title.includes("Declined") || n.title.includes("Evidence") || n.title.includes("Status:")));
      }
      if (r === "field") {
        return n.type === "field" || (n.type === "mission" && !n.title.includes("Accepted") && !n.title.includes("Declined") && !n.title.includes("Evidence"));
      }
      return false;
    });
  }

  function renderNotificationDropdownContent(role) {
    ...
    const roleLabel = currentRole === "administrator" ? "System Admin Feed" : currentRole === "command" ? "Tactical Command Feed" : "Field Patrol Feed";
      ...
      <div class="notification-dropdown-header">
        <div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
          <div style="display:flex; align-items:center; gap:8px;">
            ...
            ${unreadCount > 0 ? `<span class="badge badge-critical" style="font-size:11px; padding:2px 6px;">${unreadCount} New</span>` : ""}
          </div>
          <span class="badge badge-neutral" style="font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em;">
            ${roleLabel}
          </span>
        </div>
      </div>
        ...
        ${
          roleNotifs.length === 0
            ...
            <div style="padding:32px 16px; text-align:center; color:var(--text-muted); font-size:12px;">
              <span>📭</span>
              <p style="margin-top:6px;">No notifications for ${currentRole}.</p>
            </div>
          `
            : roleNotifs
                .map((n) => {
                  const icon =
                    n.type === "threat"
                      ...
                      : "⚡";
                  return `
            <div class="notification-item ${n.read ? '' : 'unread'}" data-notif-id="${n.id}" onclick="window.msStore.markNotificationRead('${n.id}');" style="cursor:pointer;">
      ...
      <header class="app-header">
        ...
        <div class="header-right">
          ...
          <div class="badge badge-ok" id="ml-server-status-badge" style="cursor:pointer;padding:4px 9px;font-size:11.5px;display:flex;align-items:center;gap:6px;" title="Connected to Python Flask ML Server · Random Forest Active">
            <span id="ml-server-dot" style="width:7px;height:7px;border-radius:50%;background:#10b981;box-shadow:0 0 6px #10b981;display:inline-block;transition:all 0.3s ease;"></span>
          ...
          <div class="notification-bell-wrapper">
            ...
            <div id="notif-dropdown-panel" class="notification-dropdown">
              ${renderNotificationDropdownContent(role)}
            </div>
          </div>
  ...
  function attachLayoutEvents(onTabChange) {
    ...
    let lastRenderedNotifSignature = "";
    ...
    if (notifBellBtn && notifDropdown) {
      notifBellBtn.addEventListener("click", (e) => {
        ...
        if (isOpen) {
          ...
          const rNotifs = getNotificationsForRole(window.msStore.getState(), curRole);
          lastRenderedNotifSignature = rNotifs.map((n) => n.id + ":" + (n.read ? 1 : 0)).join("|");
          notifDropdown.innerHTML = renderNotificationDropdownContent(curRole);
        }
      });
    ...
    window.msStore.subscribe((s) => {
      ...
      // Sync role-based notification badge without triggering unnecessary DOM layout thrashing
      ...
      if (notifBadge) {
        if (notifBadge.textContent !== String(unreadCount)) {
          notifBadge.textContent = unreadCount;
        }
        if (unreadCount > 0 && !notifBadge.classList.contains("has-unread")) {
          notifBadge.classList.add("has-unread");
        } else if (unreadCount === 0 && notifBadge.classList.contains("has-unread")) {
          notifBadge.classList.remove("has-unread");
        }
      }

      // If dropdown is open, re-render ONLY when notifications or their read states actually change!
      if (notifDropdown && notifDropdown.classList.contains("open")) {
        const notifSig = roleNotifs.map((n) => n.id + ":" + (n.read ? 1 : 0)).join("|");
        if (notifSig !== lastRenderedNotifSignature) {
          lastRenderedNotifSignature = notifSig;
          notifDropdown.innerHTML = renderNotificationDropdownContent(activeRole);
        }
      }
    });
    ...
    if (fusion && fusion.checkBackendHealth) {
      ...
      if (status && status.ok) {
        ...
        if (_consecutiveMlFailures >= 2) {
          ...
          badge.title = "Python ML server unreachable. Click to recheck status.";
        }
      }
    }
  }
