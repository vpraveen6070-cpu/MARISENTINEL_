...
window.MS_MAP = (function () {
  ...
  function initMap(containerId, options = {}) {
    ...
    // Marker tracking caches to preserve open popups during live simulation telemetry updates
    ...
    function updateLayers(state) {
      ...
      if (activeLayers.vessels) {
        ...
        // Build robust set of all assigned vessel identifiers across incidents, alerts, and vessels
        const assignedVesselKeys = new Set();
        (state.incidents || []).forEach((i) => {
          if (i.status !== "Closed" && (i.assignedTo || (i.missionStatus && !i.missionStatus.includes("Declined")))) {
        ...
        filteredVessels.forEach((v) => {
          ...
          // Rule Engine (Explanation only) - Correlated to AI predicted threat
          const ruleEngine = (typeof window !== "undefined" && window.MS_RULE_ENGINE) ? window.MS_RULE_ENGINE : null;
          const ruleEval = ruleEngine ? ruleEngine.evaluateRules(v, threatType) : { triggeredRules: v.reasons || [], explainability: v.explainability || "" };
          const triggeredRules = (ruleEval && ruleEval.triggeredRules && ruleEval.triggeredRules.length)
            ? ruleEval.triggeredRules
            : ((v.reasons && v.reasons.length) ? v.reasons : []);
          const ruleLogic = (ruleEval && ruleEval.logic) ? ruleEval.logic : (v.ruleLogic || triggeredRules[0] || "Route Deviation + Near Border");
          const explainability = (ruleEval && ruleEval.explainability) ? ruleEval.explainability : (v.explainability || "");
          ...
          const popupHtml = `
            <div class="ms-tactical-popup" style="font-family:Inter,sans-serif; min-width:320px; max-width:370px; padding:4px;">
              <!-- Header with clear spacing for close button -->
              <div style="display:flex; justify-content:space-between; align-items:flex-start; border-bottom:1px solid #e2e8f0; padding-bottom:6px; margin-bottom:8px; padding-right:24px;">
                <div>
              ...
              <!-- TRIGGERED RULES & EXPLAINABILITY SECTION -->
              <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:6px; padding:8px 10px; margin-bottom:8px;">
                <div style="margin-bottom:6px;">
                  ...
                  ${triggeredRules && triggeredRules.length ? `
                    <div style="font-size:11.5px; font-weight:700; color:#991b1b; display:flex; align-items:center; gap:5px; background:#fff1f2; padding:3px 7px; border-radius:4px; border:1px solid #fecaca;">
                      <span>⚠️</span> <span>${ruleLogic}</span>
                    </div>
                  ` : `
                    <div style="font-size:11px; color:#16a34a; background:#f0fdf4; padding:3px 6px; border-radius:4px;">
                ...
                ${explainability ? `
                  <div style="margin-bottom:6px;">
                    <div style="font-size:10px; font-weight:700; color:#64748b; margin-bottom:2px; text-transform:uppercase;">
                      EXPLAINABILITY:
                    </div>
                    <div style="font-size:11px; color:#334155; line-height:1.45; background:#ffffff; padding:6px 8px; border-radius:4px; border:1px solid #e2e8f0;">
                      ${explainability}
                    </div>
                  </div>
                ` : ''}
          ...
          const popupOptions = {
            ...
            closeOnClick: false,
            closeButton: true,
            className: "ms-tactical-leaflet-popup"
          };

          if (vesselMarkersMap.has(vId)) {
            const m = vesselMarkersMap.get(vId);
            ...
            m.addTo(vesselLayer);
            vesselMarkersMap.set(vId, m);
          }
        });

        // Clean up markers that are no longer present or got assigned/removed
        for (const [vId, m] of vesselMarkersMap.entries()) {
          if (!currentVesselIds.has(vId)) {
            vesselLayer.removeLayer(m);
            vesselMarkersMap.delete(vId);
          }
        }
      } else {
      ...
      if (activeLayers.incidents) {
        const activeIncidents = (state.incidents || []).filter((i) => {
          ...
          const isAssigned = i.assignedTo || (i.missionStatus && !i.missionStatus.includes("Declined"));
          if (isAssigned) return false;
          return true;
        });

        const currentIncIds = new Set();

        activeIncidents.forEach((inc) => {
          const incId = String(inc.id);
          currentIncIds.add(incId);
          const pinHtml = `
          ...
          const incPopupHtml = `
            <div class="ms-tactical-popup" style="font-family:Inter,sans-serif; min-width:240px; padding-right:20px;">
          ...
          if (incidentMarkersMap.has(incId)) {
            const m = incidentMarkersMap.get(incId);
            m.setLatLng([inc.lat, inc.lng]);
            m.setPopupContent(incPopupHtml);
          } else {
            const marker = L.marker([inc.lat, inc.lng], { icon });
            marker.bindPopup(incPopupHtml, popupOptions);
            marker.addTo(incidentLayer);
            incidentMarkersMap.set(incId, marker);
          }
        });

        for (const [incId, m] of incidentMarkersMap.entries()) {
          if (!currentIncIds.has(incId)) {
            incidentLayer.removeLayer(m);
            incidentMarkersMap.delete(incId);
          }
        }
      } else {
    ...
    bodyEl.innerHTML = `
      ...
      <div style="background:var(--bg-card-subtle); border:1px solid var(--border-color); border-radius:var(--radius-md); padding:14px; margin-bottom:16px;">
        ...
        <div style="display:flex; justify-content:space-between; font-size:11px; color:#64748b;">
          ...
          <span>Inference Confidence: <strong style="color:var(--text-main);">${confidence}%</strong></span>
        </div>
      </div>
      ...
      <div style="margin-top:16px;">
        ...
        <div style="display:flex; flex-direction:column; gap:8px;">
          ${
            triggeredRules && triggeredRules.length
              ? `
                <div style="display:flex; align-items:center; justify-content:space-between; background:var(--bg-card-subtle); padding:10px 14px; border-radius:var(--radius-md); border-left:3px solid ${color};">
                  <span style="font-size:13.5px; font-weight:700; color:${color};">⚠️ ${ruleLogic}</span>
                  <span class="badge" style="background:#fee2e2; color:#991b1b; font-size:11px;">Rule Corroboration</span>
                </div>
                ${explainability ? `
                  <div style="background:var(--bg-card-subtle); padding:10px 14px; border-radius:var(--radius-md); border:1px solid var(--border-color);">
                    <div style="font-size:10.5px; font-weight:700; text-transform:uppercase; color:var(--text-muted); margin-bottom:4px; letter-spacing:0.04em;">EXPLAINABILITY:</div>
                    <div style="font-size:13px; color:var(--text-main); line-height:1.5;">${explainability}</div>
                  </div>
                ` : ''}
              `
              : `
